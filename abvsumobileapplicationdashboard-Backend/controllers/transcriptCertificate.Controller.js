...
const TranscriptCertificate = require("../models/TranscriptCertificate");
const TranscriptCertificateOtp = require("../models/TranscriptCertificateOtp");
const {
  jsonError,
...
const sendOtp = async (req, res) => {
  try {
    ...
    await TranscriptCertificateOtp.findOneAndUpdate(
    ...
    const subject = "SGTU Transcript Certificate OTP";
    const text = `Your OTP for Transcript Certificate is ${otp}. It is valid for ${OTP_TTL_MINUTES} minutes.`;
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const otpRecord = await TranscriptCertificateOtp.findOne({
...
const applyForTranscriptCertificate = async (req, res) => {
  try {
    const {
      email,
      acceptUndertaking,
    ...
    const existingActiveApplication = await TranscriptCertificate.findOne({
      admission: req.user.id,
    ...
    if (existingActiveApplication) {
      return res.status(409).json({
        message:
          "You have already applied for transcript certificate. New application is allowed only if previous one is rejected.",
      });
    }
    ...
    const amount = modeOfDelivery === "Express" ? 15000 : 10000;
    ...
    const order_id = `TC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/transcript-certificate/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/transcript-certificate/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/transcript-certificate/status?cancelled=true`;
    ...
    const rzpOrder = await createRazorpayOrder({
      amountRupees: amount,
      receipt: order_id,
      notes: { service: "transcript-certificate", local_order_id: order_id },
    });
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Transcript Certificate",
      payment_purpose: "Payment for Transcript Certificate",
      status_message: "Payment initiated for Transcript Certificate",
    });

    await TranscriptCertificate.create({
    ...
    const formHTML = buildPaymentForm({
      ...
      description: "Transcript Certificate",
    ...
    console.error("Error applying for transcript certificate:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `transcript-certificate-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("TRANSCRIPT CERTIFICATE PAYMENT SLIP", {
          align: "center",
        });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text("Type: Transcript Certificate");
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const record = await TranscriptCertificate.findOne({
      payment: payment._id,
    });
    ...
    const appBase = `${appScheme}://services/transcript-certificate/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/transcript-certificate/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/transcript-certificate/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
...
const retryPayment = async (req, res) => {
  try {
    const record = await TranscriptCertificate.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await TranscriptCertificate.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await TranscriptCertificate.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `transcript-certificate-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `transcript-certificate-payment-slip-${record.enrollmentNumber}.pdf`;
