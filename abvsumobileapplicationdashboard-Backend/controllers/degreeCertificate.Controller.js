...
const DegreeCertificate = require("../models/DegreeCertificate");
const {
  jsonError,
...
const FIXED_DEGREE_CERTIFICATE_FEE = 15000;
...
const applyForDegreeCertificate = async (req, res) => {
  try {
    const {
      email,
      acceptUndertaking,
    ...
    const existingActiveApplication = await DegreeCertificate.findOne({
    ...
    if (existingActiveApplication) {
      return res.status(409).json({
        message:
          "You have already applied for degree certificate. New application is allowed only if previous one is rejected.",
      });
    }
    ...
    const amount = FIXED_DEGREE_CERTIFICATE_FEE;
    ...
    const order_id = `DC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/degree-certificate/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/degree-certificate/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/degree-certificate/status?cancelled=true`;
    if (!keyId || !keySecret) {
      return res
        .status(500)
    ...
    const rzpOrder = await createRazorpayOrder({
      amountRupees: amount,
      receipt: order_id,
      notes: { service: "degree-certificate", local_order_id: order_id },
    });
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Degree Certificate",
      payment_purpose: "Payment for Degree Certificate",
      status_message: "Payment initiated for Degree Certificate",
    });

    await DegreeCertificate.create({
    ...
    console.error("Error applying for Degree Certificate:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `degree-certificate-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("Degree Certificate PAYMENT SLIP", { align: "center" });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text("Type: Degree Certificate");
      doc.text("Delivery Mode: Standard");
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const record = await DegreeCertificate.findOne({ payment: payment._id });
    ...
    const appBase = `${appScheme}://services/degree-certificate/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/degree-certificate/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/degree-certificate/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
...
const retryPayment = async (req, res) => {
  try {
    const record = await DegreeCertificate.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await DegreeCertificate.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await DegreeCertificate.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `degree-certificate-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `degree-certificate-payment-slip-${record.enrollmentNumber}.pdf`;
