...
const ProvisionalDegreeCertificate = require("../models/ProvisionalDegreeCertificate");
const ProvisionalDegreeCertificateOtp = require("../models/ProvisionalDegreeCertificateOtp");
const {
  jsonError,
...
const sendOtp = async (req, res) => {
  try {
    ...
    await ProvisionalDegreeCertificateOtp.findOneAndUpdate(
    ...
    const subject = "SGTU Provisional Degree Certificate OTP";
    const text = `Your OTP for Provisional Degree Certificate is ${otp}. It is valid for ${OTP_TTL_MINUTES} minutes.`;
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const otpRecord = await ProvisionalDegreeCertificateOtp.findOne({
...
const applyForProvisionalDegreeCertificate = async (req, res) => {
  try {
    const {
      email,
      acceptUndertaking,
    ...
    const existingActiveApplication =
      await ProvisionalDegreeCertificate.findOne({
        admission: req.user.id,
    ...
    if (existingActiveApplication) {
      return res.status(409).json({
        message:
          "You have already applied for provisional degree certificate. New application is allowed only if previous one is rejected.",
      });
    }

    const marksheetFile = req.files?.marksheetFile?.[0] || null;
    const lastMigrationFile = req.files?.lastMigrationFile?.[0] || null;
    ...
    if (!lastMigrationFile) {
      return res
        .status(400)
        .json({ message: "Last migration PDF is mandatory" });
    }

    if (marksheetFile.size > MAX_FILE_BYTES) {
      return res
        .status(400)
        .json({ message: "Marksheet PDF must be less than 200KB" });
    }

    if (lastMigrationFile.size > MAX_FILE_BYTES) {
      return res
        .status(400)
        .json({ message: "Last migration PDF must be less than 200KB" });
    }
    ...
    const amount = modeOfDelivery === "Express" ? 5000 : 3000;
    ...
    const order_id = `PDC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/provisional-degree-certificate/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/provisional-degree-certificate/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/provisional-degree-certificate/status?cancelled=true`;
    if (!keyId || !keySecret) {
      return res
        .status(500)
    ...
    const rzpOrder = await createRazorpayOrder({
      ...
      receipt: order_id,
      notes: {
        service: "provisional-degree-certificate",
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Provisional Degree Certificate",
      payment_purpose: "Payment for Provisional Degree Certificate",
      status_message: "Payment initiated for Provisional Degree Certificate",
    });

    await ProvisionalDegreeCertificate.create({
      ...
      lastMigrationFileUrl: normalizeFile(lastMigrationFile),
    ...
    const formHTML = buildPaymentForm({
      ...
      name: "SGTU",
      description: "Provisional Degree Certificate",
    ...
    console.error("Error applying for provisional degree certificate:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `provisional-degree-certificate-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("PROVISIONAL DEGREE CERTIFICATE PAYMENT SLIP", {
          align: "center",
        });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text("Type: Provisional Degree Certificate");
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const record = await ProvisionalDegreeCertificate.findOne({
      payment: payment._id,
    });
    ...
    const appBase = `${appScheme}://services/provisional-degree-certificate/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/provisional-degree-certificate/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/provisional-degree-certificate/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
...
const retryPayment = async (req, res) => {
  try {
    const record = await ProvisionalDegreeCertificate.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await ProvisionalDegreeCertificate.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await ProvisionalDegreeCertificate.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `provisional-degree-certificate-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `provisional-degree-certificate-payment-slip-${record.enrollmentNumber}.pdf`;
