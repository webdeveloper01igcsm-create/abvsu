...
const CharacterCertificate = require("../models/CharacterCertificate");
const CharacterCertificateOtp = require("../models/CharacterCertificateOtp");
const {
  jsonError,
...
const sendOtp = async (req, res) => {
  try {
    ...
    await CharacterCertificateOtp.findOneAndUpdate(
    ...
    const subject = "SGTU Character Certificate OTP";
    const text = `Your OTP for Character Certificate is ${otp}. It is valid for ${OTP_TTL_MINUTES} minutes.`;
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const otpRecord = await CharacterCertificateOtp.findOne({
...
const applyForCharacterCertificate = async (req, res) => {
  try {
    const {
      email,
      acceptUndertaking,
    ...
    // change amount to update the fees for character certificate based on delivery mode
    const amount = modeOfDelivery === "Express" ? 3500 : 2000;
    ...
    const order_id = `CC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/character-certificate/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/character-certificate/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/character-certificate/status?cancelled=true`;
    ...
    const rzpOrder = await createRazorpayOrder({
      amountRupees: amount,
      receipt: order_id,
      notes: { service: "character-certificate", local_order_id: order_id },
    });
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Character Certificate",
      payment_purpose: "Payment for Character Certificate",
      status_message: "Payment initiated for Character Certificate",
    });

    await CharacterCertificate.create({
    ...
    console.error("Error applying for character certificate:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `character-certificate-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("CHARACTER CERTIFICATE PAYMENT SLIP", { align: "center" });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text("Type: Character Certificate");
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const params = {
      ...
      order_status: isCancelledQuery ? "Cancelled" : verifiedOrderStatus,
    ...
    const appBase = `${appScheme}://services/character-certificate/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/character-certificate/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/character-certificate/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
...
const retryPayment = async (req, res) => {
  try {
    const record = await CharacterCertificate.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await CharacterCertificate.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await CharacterCertificate.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `character-certificate-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `character-certificate-payment-slip-${record.enrollmentNumber}.pdf`;
