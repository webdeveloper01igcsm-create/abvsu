...
const CourseCompletionCertificate = require("../models/CourseCompletionCertificate");
const CourseCompletionCertificateOtp = require("../models/CourseCompletionCertificateOtp");
const {
  jsonError,
...
const sendOtp = async (req, res) => {
  try {
    ...
    await CourseCompletionCertificateOtp.findOneAndUpdate(
    ...
    const subject = "SGTU Course Completion Certificate OTP";
    const text = `Your OTP for Course Completion Certificate is ${otp}. It is valid for ${OTP_TTL_MINUTES} minutes.`;
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const otpRecord = await CourseCompletionCertificateOtp.findOne({
...
const applyForCourseCompletionCertificate = async (req, res) => {
  try {
    const {
      email,
      acceptUndertaking,
    ...
    const existingActiveApplication = await CourseCompletionCertificate.findOne(
      {
        admission: req.user.id,
    ...
    if (existingActiveApplication) {
      return res.status(409).json({
        message:
          "You have already applied for course completion certificate. New application is allowed only if previous one is rejected.",
      });
    }
    ...
    const order_id = `CCC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/course-completion-certificate/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/course-completion-certificate/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/course-completion-certificate/status?cancelled=true`;
    if (!keyId || !keySecret) {
      return res
        .status(500)
    ...
    const rzpOrder = await createRazorpayOrder({
      ...
      receipt: order_id,
      notes: {
        service: "course-completion-certificate",
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Course Completion Certificate",
      payment_purpose: "Payment for Course Completion Certificate",
      status_message: "Payment initiated for Course Completion Certificate",
    });

    await CourseCompletionCertificate.create({
    ...
    console.error("Error applying for course completion certificate:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `course-completion-certificate-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("COURSE COMPLETION CERTIFICATE PAYMENT SLIP", {
          align: "center",
        });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text("Type: Course Completion Certificate");
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const record = await CourseCompletionCertificate.findOne({
      payment: payment._id,
    });
    ...
    const appBase = `${appScheme}://services/course-completion-certificate/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/course-completion-certificate/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/course-completion-certificate/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
...
const retryPayment = async (req, res) => {
  try {
    const record = await CourseCompletionCertificate.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await CourseCompletionCertificate.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await CourseCompletionCertificate.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `course-completion-certificate-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `course-completion-certificate-payment-slip-${record.enrollmentNumber}.pdf`;
