...
const DuplicateDocument = require("../models/DuplicateDocument");
const DuplicateDocumentOtp = require("../models/DuplicateDocumentOtp");
const {
  jsonError,
...
const sendOtp = async (req, res) => {
  try {
    ...
    await DuplicateDocumentOtp.findOneAndUpdate(
    ...
    const subject = "SGTU Duplicate Document OTP";
    const text = `Your OTP for Duplicate Document application is ${otp}. It is valid for ${OTP_TTL_MINUTES} minutes.`;
...
const verifyOtp = async (req, res) => {
  try {
    ...
    const otpRecord = await DuplicateDocumentOtp.findOne({
...
const applyForDuplicateDocument = async (req, res) => {
  try {
    const {
      email,
    ...
    if (!duplicateDocumentType) {
      return res
        .status(400)
        .json({ message: "Please select duplicate document type" });
    }

    const existingActiveApplication = await DuplicateDocument.findOne({
      admission: req.user.id,
      duplicateDocumentType,
    ...
    if (existingActiveApplication) {
      return res.status(409).json({
        message:
          "You already have an active duplicate document request for this document type. New application is allowed only if previous one is rejected.",
      });
    }

    const affidavitFile = req.files?.affidavitFile?.[0] || null;

    if (!affidavitFile) {
      return res.status(400).json({ message: "Affidavit PDF is required" });
    }

    if (affidavitFile.size > MAX_FILE_BYTES) {
      return res
        .status(400)
        .json({ message: "Affidavit PDF must be less than 200KB" });
    }
    ...
    //as per discussion with office, normal delivery will be free and express will be paid, so amount is decided based on delivery mode
    const amount = modeOfDelivery === "Express" ? 3000 : 2000;
    ...
    const order_id = `DDC${Date.now()}${uuidv4().substring(0, 6)}`;
    const currency = "INR";
    ...
    const redirect_url = `${apiBase}/duplicate-document/razorpay-response${returnQuery}`;
    const cancel_url = isMobileReturn
      ? `${apiBase}/duplicate-document/razorpay-response${returnQuery}`
      : `${studentBase}/authenticate/duplicate-document/status?cancelled=true`;
    ...
    const rzpOrder = await createRazorpayOrder({
      amountRupees: amount,
      receipt: order_id,
      notes: { service: "duplicate-document", local_order_id: order_id },
    });
    ...
    const payment = await Payment.create({
      ...
      payment_source: "Duplicate Document",
      payment_purpose: "Payment for Duplicate Document",
      status_message: "Payment initiated for Duplicate Document",
    });

    await DuplicateDocument.create({
      ...
      emailVerified: true,
      duplicateDocumentType,
      affidavitFileUrl: normalizeFile(affidavitFile),
    ...
    console.error("Error applying for duplicate document:", error);
...
const generatePaymentSlip = (record, payment) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fileName = `duplicate-document-payment-slip-${record._id}.pdf`;
      ...
      doc
        .fontSize(20)
        .font("Helvetica-Bold")
        .text("DUPLICATE DOCUMENT PAYMENT SLIP", {
          align: "center",
        });
      doc.moveDown();
      ...
      doc.font("Helvetica");
      doc.text(`Type: ${record.duplicateDocumentType}`);
      doc.text(`Delivery Mode: ${record.modeOfDelivery}`);
      doc.text(`Amount Paid: G�${record.amount}`);
      doc.moveDown(2);
...
const handleRazorpayResponse = async (req, res) => {
  try {
    ...
    const record = await DuplicateDocument.findOne({
      payment: payment._id,
    });
    ...
    const appBase = `${appScheme}://services/duplicate-document/status`;
    let redirectUrl;
    ...
    if (isSuccess) {
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/duplicate-document/status?payment=success&orderId=${encodeURIComponent(
            params.order_id || "",
          )}`;
    } else if (isCancelled) {
      redirectUrl = isMobileReturn
      ...
      redirectUrl = isMobileReturn
        ...
        : `${studentBase}/authenticate/duplicate-document/status?payment=failed&orderId=${encodeURIComponent(
            params.order_id || "",
          )}&reason=${failureReason}`;
    }
    ...
    return res.status(200).send(`
      <html>
        ...
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 40px; color: #1f2937;">
          ...
          <p style="margin-bottom: 8px;">Amount Paid: G�${paidAmount}</p>
...
const retryPayment = async (req, res) => {
  try {
    const record = await DuplicateDocument.findById(req.params.id);
    if (!record) {
...
const getMyRecords = async (req, res) => {
  try {
    const records = await DuplicateDocument.find({
      admission: req.user.id,
    }).sort({ createdAt: -1 });
...
const updateRecord = async (req, res) => {
  try {
    ...
    const record = await DuplicateDocument.findByIdAndUpdate(
      req.params.id,
      updates,
      { new: true },
    );
...
const downloadGeneratedFile = async (req, res) => {
  try {
    ...
    const inlineName = sanitizePdfFilename(
      `duplicate-document-${record.enrollmentNumber || record._id}`,
    );
...
const getPaymentSlip = async (req, res) => {
  try {
    ...
    const downloadName = `duplicate-document-payment-slip-${record.enrollmentNumber}.pdf`;
