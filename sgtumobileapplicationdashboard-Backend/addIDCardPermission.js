...
async function addIDCardPermission() {
  let connection;
  try {
    ...
    for (const user of adminUsers) {
      ...
      const hasPermission = normalizedPermissions.some(
        (perm) => perm.module === "Duplicate Document Management",
      );

      if (!hasPermission) {
        normalizedPermissions.push({
          _id: new mongoose.Types.ObjectId(),
          module: "Duplicate Document Management",
          view: true,
          write: true,
          update: true,
          delete: true,
        ...
        updated++;
        console.log(
          `✅ Added Migration Certificate Management permission to: ${user.email}`,
        );
      } else {
        console.log(
          `⏭️  User ${user.email} already has Migration Certificate Management permission`,
        );
      }
      ...
      if (!userSanitized && hasPermission) {
        continue;
      }
    }
