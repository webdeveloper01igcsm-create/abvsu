...
async function createProvisionalCertificate(applicationData) {
  const templatePath = path.join(
    process.cwd(),
    "lib",
    "AtalPROVISIONALCERTIFICATE.pdf"
  );
...
async function createCourseCompletionCertificate(applicationData) {
  const templatePath = path.join(
    process.cwd(),
    "lib",
    "atalcoursecompletetioncertificate.pdf"
  );
  const existingPdfBytes = fs.readFileSync(templatePath);
  ...
  const cinzelBytes = fs.readFileSync(
    path.join(process.cwd(), "/utils/fonts/SortsMillGoudy-Regular.ttf")
  );
...
async function createMigrationCertificate(applicationData) {
  const templatePath = path.join(
    process.cwd(),
    "lib",
    "AtalMIGRATIONCERTIFICATE.pdf"
...
async function createSecondDegreeCertificate(applicationData) {
